...
function buildRequest() {
    ...
    //THE EVENTUAL BILL ADD REQUEST HERE:

    //Given a readytopayids, query mongo for the full invoice objects,
    //and convert to an XML request:

    // var count = 0;
    // 
    // for (var i = 0; i < readytopayids.length; i++) {
    //     request[i] = builder.create('QBXML', { version: '1.0' })
    //             .instruction('qbxml', 'version="4.0"')
    //             .ele('QBXMLMsgsRq', { 'onError': 'stopOnError' })
    //                 .ele('BillPaymentCheckAdd', {'requestID': i.toString()})
    //                     .ele('MaxReturned')
    //                         .txt('1');
    // }
    //
...
module.exports.run = function runQBWS() {
    ...
    soapServer = soap.listen(server, '/', qbws, xml);
