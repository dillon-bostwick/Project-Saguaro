...
var loadFromDropzone = (token, callback) => {
	...
	dropboxConnection.getDirList(globals.dropzonePath, (err, dirList) => {
		if (err) 				callback(err);
		if (_.isEmpty(dirList)) callback(null);

		//filter out folders etc
		let files = _.where(dirList.entries, { '.tag': 'file' });

		if (_.isEmpty(files)) callback(null);
...
var doProcessFileToActive = (dropboxConnection) => {
	...
	return (file, callback) => {
		...
		var tasks = [
			...
			(callback) => { generateInvFromFile(file).save(callback) }
		];
...
function generateInvFromFile(file) {
	...
	return new Invoice({
		...
		lineItems: [],
		actions: [{
			desc: 'File uploaded to the Dropbox ' + copyrightChar + 'Dropzone',
