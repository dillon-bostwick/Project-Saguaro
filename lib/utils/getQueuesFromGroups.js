...
var getQueuesFromGroups = (groupIds, callback) => {
	...
	Group.find({}, (err, allGroups) => {
		...
		if (_.isEmpty(targetGroups)) {
			callback('No group matches found. IDs might be outdated or malformed', null);
		}

		async.each(targetGroups, getGroupQueue, callback);
	});

	callback('Database connection failed', null);
}
...
var getGroupQueue = (group, callback) => {
	...
	async.each(invoiceIds, (invoiceId, callback) => {
		Invoice.findById(invoiceId, (err, invoice) => {
			if (err) callback(err, null);
			if (!invoice) callback('Cant find invoice ' + invoiceId);
