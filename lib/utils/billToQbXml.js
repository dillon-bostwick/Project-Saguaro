...
var billToQbXml = (populatedInvoice, requestId) => {
    assert(_.isString(requestId) || _.isNumber(requestId));

    if (_.isNumber(requestId)) requestId = requestId.toString();
	...
	_.each(qbExpenseLines, (expenseLine) => {
		qbXml.ele('ExpenseLineAdd')
				.ele('AccountRef')
                    .ele('FullName')
                        .text(expenseLine._expense.name || 'Warranty')
                    .up()
                .up()
                .ele('Amount')
                ...
                .up()
                .ele('ClassRef')
                    .ele('FullName') // show hood/lot if warranty
                        .text(expenseLine.type === 'WARRANTY'
