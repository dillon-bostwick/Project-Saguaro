 ...
 *
 * Possible client errors: none. Due to nature of request any error is internal
 * thus sends 500
 */
...
var getOwnQueues = (req, res, next) => {
	var groupIds = req.user._groups;
	var queues = [];

	var tasks = user.canKey ? [getKeyQueue(), getGroupsQueues(groupIds)]
						    : [getGroupsQueues(groupIds)];
 ...
 */
function getGroupsQueues(groupIds) {
	return (callback) => {
 ...
 * @return {[type]} [description]
 */
function getKeyQueue() {
	return (callback) => {
		utils.loadNewFromDropzone(user.currentToken, (err) => {
			if (err) callback(err, null);

			utils.getKeyableInvoices(callback);
		});
	};
}
